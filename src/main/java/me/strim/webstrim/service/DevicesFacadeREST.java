...
public class DevicesFacadeREST extends AbstractFacade<Devices> {
    ...
    @Path("/secured/update")
    @Consumes(MediaType.APPLICATION_JSON)
    public void edit(Devices entity) {
        super.edit(entity);
    }
    
    @PUT
    @Path("/secured/update/strimStatus/{id}/{status}")
    @Consumes(MediaType.APPLICATION_JSON)
    public void editStatus(@PathParam("id") Integer id, @PathParam("status") Boolean status) {
        Devices dev =super.find(id);
        dev.setStrimStatus(status);
        super.edit(dev);
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public List<Devices> find(@PathParam("username") String username) {
        return (List<Devices>) em.createNamedQuery("Devices.findByUsername").setParameter("username", username).getResultList();
    }
